...
DB.on("enqueue", (connection) => {
  console.log(
    "│  ├── DB :: queued to wait for an available connection %d",
    connection.threadId
  );
});
...
CoreDB.createFromMap = (data) => {
  return new Promise((resolve, reject) => {
    ...
    if (data.IdPembelian != null) {
      ...
      let valuess = Object.values(data);
      console.log(keys);
      let fields = [];
      let vals = [];
...
CoreDB.deleteArray = (condition) => {
  return new Promise((resolve, reject) => {
    ...
    if (condition.length > 0) {
      condition.map(function (val, key) {
        fields.push(`${val.key} IN (${val.value})`);
      });
